...
This repo includes `.github/workflows/build-apk.yml`, which builds an Android debug APK on every push to `main` (or manually via the Actions tab → "Run workflow"). Capacitor (`@capacitor/core`, `@capacitor/cli`, `@capacitor/android`) is already listed in `package.json`, and `capacitor.config.json` is already set up, so you don't need Android Studio or any local setup — just push the repo as-is and the workflow does everything: installs dependencies, builds the web game, creates the Android project, and builds the APK.

Find the finished APK under the Actions tab → the latest "Build Android APK" run → the `app-debug-apk` artifact at the bottom of the run page.

If you ever want to generate the Android project locally instead (e.g. to open it in Android Studio), you can still run:
